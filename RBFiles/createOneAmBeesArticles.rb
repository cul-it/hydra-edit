   ...
   #   keywords << k.content() << " "
    ...
    @doc.xpath(sprintf('//%s',tagname)).each do |record|
       ...
       if head == 0 or head.nil?
         ...
         articlespid = "hivebees" + "6366245" + "_" + volume_number_text + "_" + issue_number_text + "_articles_"
         issuepid = "hivebees" + "6366245" + "_" + volume_number_text + "_" + issue_number_text

       end
       articleCount = 0
       record.xpath('.//DIV2').each do |ab|
         articleCount = articleCount + 1
         artpid = articlespid + articleCount.to_s
         #puts ab.xpath('.//AUTHOR').inspect
         puts "PID = " + artpid
         titlenew = getChildNodeContents(ab,"HEAD/TITLE")
         authornew = getChildNodeContents(ab,"HEAD/AUTHOR")
         puts "Title = " + titlenew
         if authornew == 0
           authornew = ""
         end
         puts "Author = " + authornew

       #uncomment next line and line 223 if picking up after a fail 
        if articleCount >= 6
         article = Article.new(id: artpid, title: [titlenew], creator: [authornew])
     #    article = Article.find(artpid)
#         article = Article.find(artpid)
         article.apply_depositor_metadata("jac244@cornell.edu")
      
         ab.xpath('PB1').each do |pb1|
            image_seq = " "
            image_seq = pb1.values[1]
            image_seq.sub!(/^0+/,"")   
            puts "Article pagepid = " + issuepid + "_" + image_seq 
            pagepid = issuepid + "_" + image_seq
            page = Page.find(pagepid)
            page.apply_depositor_metadata("jac244@cornell.edu")
            article.members << page
         end
...
inputparam = ARGV[0]
seq_array = []
lines = File.foreach("/collections/bees/beearticlesSingle.txt")
lines.each do |line|
