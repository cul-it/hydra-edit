...
class Parser
  ...
  def parseRecords(tagname, seq_array)
    ...
    puts "Seq_array = " + seq_array.inspect
    pagepid = ''
    volume_number_text = ''
    issue_number_text = ''
    bibID = ""
...
#    puts "START OF PAGES"
    head = ""
      ...
      if vol_issue_text[2] == 'Supplement '
        issue_text = vol_issue_text[1].split(' ')
        issue_number_text = issue_text[1] + '_supplement'
      else
        issue_text = vol_issue_text[1].split(' ')
   ...
   #   keywords << k.content() << " "
    ...
    @doc.xpath(sprintf('//%s',tagname)).each do |record|
       ...
       record.xpath('PB1').each do |pb1|
	      ...
	      if image_seq.to_i >= 4 #and image_seq.to_i <= 12
...
inputparam = ARGV[0]
seq_array = []
lines = File.foreach("/collections/bees/beefilesOne.txt")
lines.each do |line|
