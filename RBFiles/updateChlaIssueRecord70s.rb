...
#    note = []
    ...
    #       puts "titlestmt_titletype = " + titlestmt_titletype
      ...
      #   if !vol_issue_text[1].nil?
      #      series_number_text = vol_issue_text[1].split(' ')[1]
      #      puts "series_number_text = " + series_number_text
     #   end
         if vol_issue_text[1] == 'Index '
           issue_number_text = 'index'
         else
  #        if !vol_issue_text[1].nil? and !vol_issue_text[1].include? "Edition Revised"
  ...
  #        end
           ...
           if !vol_issue_text[1].nil? and (vol_issue_text[1].include? "Series War Emergency Bulletin")
               bulletin_text = 'SWEB'
               textArray = vol_issue_text[1].split(' ')
               bulletin_number_text = textArray[4]
           end
           if !vol_issue_text[1].nil? and (vol_issue_text[1].include? "Edition Revised" or vol_issue_text[1].include?"Edition Reprint")
             issue_number_text = issue_number_text + "_revised"
           else
             if !vol_issue_text[2].nil? and vol_issue_text[2].include? "Supplement"
              issue_number_text = issue_number_text + "_supplement"
             else
              if issue_number_text.nil? or issue_number_text == ""
                issue_number_text = "1" #series_number_text #""
              end
             end
           end
         end
         ...
         puts "bulletin text = " + bulletin_text + " " + bulletin_number_text
...
#         end
  #       book_ocr << ""
     ...
     #    puts book_ocr
         ...
         journalpid = "chla" + journalpid
#        col = Collection.find("corehist")
         puts "INT = " + issue_number_text 
         if issue_number_text.blank?
            issuepid = journalpid + "_" + volume_number_text + "_" + issue_number_text 
         else
            issuepid = journalpid + "_" + volume_number_text + "_" + issue_number_text #bulletin_text + "_" + bulletin_number_text #issue_number_text 
         end
    #    laststring = pubstmt_idno.split('_')[2]
    #    lastnum = laststring.to_i + 93
    #    puts lastnum.to_s
##        issuepid = "chla7031969_47_" + lastnum.to_s
        issuepid = "chla7031969_142_1"
        puts " finding info for " + issuepid 
        issue = Issue.find(issuepid)
        issue.our_identifier = [pubstmt_idno]
##        issue = Issue.new(id: issuepid, title: [titlestmt_title], editor: [editor], extent: [extent], 
##                           format: ["Issue"], format_URI: [format_URI],  
##                           issue_type: [item_type], issue_type_URI: [item_type_URI], edition: [edition], 
...
##                           related_url: [related_url], pubplace: [pubstmt_pubplace], identifier: [issuepid], our_identifier: [pubstmt_idno]) 
    #   issue.apply_depositor_metadata("jac244@cornell.edu")
##       journal = Journal.find(journalpid)
...
inputparam = ARGV[0]
lines = File.foreach("updatesForWednesday70s.txt")
lines.each do |line|
 inputparam = line.chomp
 ARGV[0] = line.chomp
 data = Parser.new("/collections/chla/" + inputparam + "/chla-s-70-" + inputparam + "-serials-WithDimsOCR-Mar15.xml")
