...
class Parser
  ...
  def initialize(filename, inputID)
  ...
  def parseRecords(tagname, inputID)
    bibID = ""
    callno = ""
    keywords = []
    titlestmt_titletype = ""
...
#            image_seq = " " 
       ...
       #   puts ARGV[1] 
          if image_seq.to_i >= ARGV[0].to_i and image_seq.to_i <= ARGV[1].to_i
          #bookid = "chla" + ARGV[0]
          pagepid = "chla" + inputID + "_" + image_seq
...
#             puts "FileSet ID = " +  filesetid
             ...
             fs.awsimage = ["http://s3.amazonaws.com/cul-hydra/chla/" + inputID + "/jpg/" + image_ref]
             fs.awsthumbnail = ["http://s3.amazonaws.com/cul-hydra/chla/" + inputID + "/thumbs/" + image_ref]
          else
...
lines.each do |line|
  ...
  inputID = line.chomp
data = Parser.new("/collections/chla/" + inputparam + "/chla-m-" + inputparam + "-monograph-WithDims-June10.xml", inputID)
data.parseRecords("DIV1", inputID)
end
