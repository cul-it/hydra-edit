   ...
   #   keywords << k.content() << " "
    ...
    @doc.xpath(sprintf('//%s',tagname)).each do |record|
     ...
     #     thumbnail = "http://hydrastg.library.cornell.edu/fedora/get/" + pagepid + "/thumbnailImage"
           ...
           book.save
            page.save
        #   page.pageImage.content = File.open("/collections/new_may/" + ARGV[0] +"/jpg/" + image_ref)
        #   page.pageImageThumbnail.content = File.open("/collections/new_may/" + ARGV[0] +"/thumbs/" + image_ref)
            image_format = ""
            image_geo = ""
            image_date = ""
            image_ethnic = ""
...
#  exit
#end
lines = File.foreach("mayonebookpages.txt")
lines.each do |line|
  inputparam = line.chomp
  ARGV[0] = line.chomp
data = Parser.new("/collections/new_may/" + inputparam + "/" + inputparam + "_john_Jun15_dims.xml")
