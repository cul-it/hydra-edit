   ...
   #   keywords << k.content() << " "
    ...
    @doc.xpath(sprintf('//%s',tagname)).each do |record|
       ...
       record.xpath('PB1').each do |pb1|
            ...
            image_caption = ""
            image_ocr = ""
            title = image_ftr + " " + image_n + " fileset"
          ...
          if image_seq.to_i == 1
          pagepid = ARGV[0] + "_" + image_seq + "_fs"
          puts "shift"
          puts subject.to_s
     #     thumbnail = "http://hydrastg.library.cornell.edu/fedora/get/" + pagepid + "/thumbnailImage"
           fileset = FileSet.new(id: pagepid, fileset_title: [title])
            puts "swing"
           fileset.apply_depositor_metadata("jac244@cornell.edu")
           page = Page.find(ARGV[0] + "_" + image_seq)
           page.apply_depositor_metadata("jac244@cornell.edu")
           page.members << fileset
