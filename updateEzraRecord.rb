  ...
  def parseRecords(tagname)
   ...
   #   keywords << k.content() << " "
      k.xpath('.//TERM').each do |term|
        if term.content.include? "Folder "
          folderterms = term.content().split(': ')
          foldertitles << folderterms[0]
          terms = folderterms[1]
          folderterms = terms.split('; ')
          folderterms.each do |finder|
            if !keywords.include?(finder)
              keywords << finder
            end
          end
        end
...
#                          format: ["Box"], our_identifier: [bookpid], publisher: [pubstmt_publisher],
...
#                          subject: keywords)
       ...
       book.subject = keywords
       book.date_created = pubdate
     ...
     #  col.save
       #col.to_solr
     #  col.update_index
      puts "Done with update of " + bookpid
  end
end
inputparam = ARGV[0]
if inputparam.nil?
